...
| T-09 | Refresh failure causes regenerated business operation IDs | B3 runtime owns one refresh coordinator independent of later operation outbox; callers join one future; access is published only after refresh journal readback and epoch check. | Crash/race tests prove exact predecessor recovery, one concurrent refresh, 30-second expiry to login, and logout/reactivation epochs. | Later B4 must preserve immutable operation IDs across its authenticated retry. |
...
| T-24 | Backup restores credentials/database to another Device | B2 disables app backup and broadly excludes file/database/shared-preference/device domains; B3 permanently binds API/Workspace/Company/Branch/User/Device and revalidates `/auth/me` before identity-ready. | Copied/missing-key states fail closed; immutable context mismatch clears the session while preserving ciphertext/binding. | OEM restore/device-transfer remains a pilot gate; rooted/live-process compromise is not solved. |
